...
# RCS Identication ; $Revision: 8485 $ ; $Date: 2013-01-24 14:38:37 +0100 (jeu 24 jan 2013) $ 
#
...
my %in_regexp = (
		 ...
		 'family' => &tools::get_regexp('family_name'),
...
#XXX## Important to leave this there because it defined defaults for user_data_source
#XXX&List::check_db_connect();
    ...
    my $http_host = &get_header_field('HTTP_HOST');
     ...
     foreach my $p ('email','host','sympa','request','soap_url','wwsympa_url','listmaster_email','logo_html_definition',
		    ...
		    'dark_color','light_color','text_color','bg_color','error_color','use_blacklist','antispam_feature','custom_robot_parameter',
                    'selected_color','shaded_color','color_0','color_1','color_2','color_3','color_4','color_5','color_6','color_7','color_8','color_9','color_10','color_11','color_12','color_13','color_14','color_15','automatic_list_families') {
	 ...
	 $session->{'email'}= 'nobody'; $session->{'id_session'} = &SympaSession::get_random();
     }

    $session->{'is_family_owner'} = undef;
    my $automatic_list_families = &Conf::get_robot_conf($robot,'automatic_list_families');
...
#         }
	 ...
	 while ($action) {
		...
		unless($action = prevent_visibility_bypass()) {
		    ...
		    &report::reject_report_web('user','authorization_reject',{},$param->{'action'},'');
		}
	     }
	...
	unless ($param->{'bypass'} eq 'extreme') {
 	 ...
 	 ## Icons
 	 $param->{'icons_url'} = &Conf::get_robot_conf($robot, 'static_content_url').'/icons';
  ...
  if (defined $required_args{$action}) {
	...
	unless ($ok) {
	  ...
	  $in{$arg_name} =~ s/\|/,/g;
	    ...
	    if(prevent_visibility_bypass()) {
		&report::reject_report_web('user','authorization_reject',{'list' => $in{'list'}},$param->{'action'},'');
	    }
...
sub send_html {
	...
	Language::PushLang("en");
	printf "Last-Modified: %s\n", &POSIX::strftime('%a, %d %b %Y %H:%M:%S GMT', gmtime($param->{'header_date'}));
	Language::PopLang();
    }
    print "Cache-control: no-cache\n"  unless ( $param->{'action'} eq 'arc')  ;
    print "Content-Type: text/html\n";
    ## Workaround for Internet Explorer 8.
    print "X-UA-Compatible: IE=7\n";
    ...
    unless (&tt2::parse_tt2($param_copy,$tt2_file , \*STDOUT,
	    $tt2_include_path, { 'has_header' => 1 })) {
	my $error = &tt2::get_error();
	$param->{'tt2_error'} = $error;
	$param_copy->{'tt2_error'} = $error;

	if ($param->{'action'} eq 'help' and
	    ref $error and $error->type eq 'file') {
	...
	&tt2::parse_tt2($param_copy,'tt2_error.tt2' , \*STDOUT,
			$tt2_include_path, { 'has_header' => 1 });
    }
}
...
=cut 
 ...
 sub check_param_out {
     ...
     $param->{'start_time'} = gettext_strftime "%d %b %Y at %H:%M:%S", localtime($start_time);
     $param->{'process_id'} = $$;
...
sub do_ticket {
	...
	if ($param->{'ticket_context'}{'result'} eq 'success') {
	    ...
	    $param->{'last_login_date'} = gettext_strftime "%d %b %Y at %H:%M:%S", localtime($param->{'user'}{'last_login_date'}) if ($param->{'user'}{'last_login_date'}); 
 ...
 sub do_login {
	...
	if $in{'next_action'} =~ /^(login|logout)$/;
     ...
     $param->{'last_login_date'} = gettext_strftime "%d %b %Y at %H:%M:%S", localtime($data->{'user'}{'last_login_date'}) if ($data->{'user'}{'last_login_date'});
...
####################################################
 sub do_requestpasswd {
     ...
     unless ($param->{'newuser'} = &List::get_user_db($in{'email'})) {
	$param->{'newuser'} =
 ...
 sub do_lists {
	 ...
	 if (! $in{'topic'}) {
		...
		if (($in{'topic'} eq 'other' and ! $in{'subtopic'}) or
		    $in{'topic'} eq 'topicsless') {
		    $param->{'which'}{$list->{'name'}} = $list_info
			if $tree[0] eq 'other' and ! $tree[1];
		    next;
		}
	 ...
	 } elsif (($in{'topic'} eq 'other' and ! $in{'subtopic'}) or
		  $in{'topic'} eq 'topicsless') {
	     $param->{'which'}{$list->{'name'}} = $list_info;
	 }
     }
 ...
 ## Subscribers' list
 sub do_review {
     &wwslog('info', 'do_review(%s)', $in{'page'});
     my $record;
     my @users;
     my $size ;
...
sub do_search {
    ...
    ## Decode regexp to Unicode for case-insensitive match.
    my $regexp = &tools::escape_regexp(Encode::decode_utf8($param->{'filter'} || ''));
    ...
    ## Members list
    for (my $i = $list->get_first_user({'sortby' => 'email'}); $i; $i = $list->get_next_user()) {

	## Decode gecos to Unicode for case-insensitive match.
	my $gecos = Encode::decode_utf8($i->{'gecos'} || '');
	## Search filter
...
sub do_family_signoff_request {
    &wwslog('info', 'do_family_signoff_request');
    ## If the URL isn't valid, then go to home page. No need to guide the user: this function is supposed to be used by clicking on autocreated URL only.
    return &Conf::get_robot_conf($robot, 'default_home') unless $in{'email'};

    my $ticket = &Auth::create_one_time_ticket($in{'email'},$robot,'family_signoff/'.$in{'family'}.'/'.$in{'email'},$ip);
...
sub do_family_signoff {
    &wwslog('info', 'do_family_signoff');
    $param->{'signing_off_email'} = $in {'email'};
    $param->{'family'} = $in {'family'};

    unless ($in{'email'} eq $session->{'email'}) {
	&report::reject_report_web('user','cannot_do_signoff');
	&wwslog('err','do_signoff: user %s tried to unsubscribe address %s from family %s',$session->{'email'}, $in{'email'},$in {'family'});
	return undef;
    }
    unless (List::insert_delete_exclusion($in{'email'},&SympaSession::get_random(),$robot,'insert',$in{'family'})) {
	&report::reject_report_web('user','cannot_do_signoff');
	&wwslog('err','do_signoff: Unsubscription of address %s from family %s failed.',$in{'email'},$in {'family'});
	return undef;
    }
    return 1;
}
...
####################################################### 
 sub do_modindex {
	 ...
	 $param->{'spool'}{$id}{'subject'} =  &tools::decode_header($mail, 'Subject');
	 $param->{'spool'}{$id}{'subject'} ||= 'no_subject';
	 $param->{'spool'}{$id}{'date'} = &tools::decode_header($mail, 'Date');
	 $param->{'spool'}{$id}{'from'} = &tools::decode_header($mail, 'From');
 ...
 sub do_reject {
    ...
    &wwslog('info', 'do_reject(%s)', $in{'id'});
	 ...
	 if (($in{'quiet'} ne '1')||($in{'blacklist'})) {
		 ...
		 unless ($in{'quiet'}) {
		     my %context;
		     $context{'subject'} = &tools::decode_header($msg, 'Subject');
		     $context{'rejected_by'} = $param->{'user'}{'email'};
...
## TODO : edit family file ???
 sub do_editfile {
	 ...
	 foreach my $f ('info','homepage','welcome.tt2','bye.tt2','removed.tt2','message.footer','message.header','remind.tt2','invite.tt2','reject.tt2','your_infected_msg.tt2') {
	     ...
	     next unless ($list->may_edit($filename_for_auth, $param->{'user'}{'email'}) eq 'write');
	     if ($wwslib::filenames{$f}{'gettext_id'}) {
		 $param->{'files'}{$f}{'complete'} = gettext($wwslib::filenames{$f}{'gettext_id'});
	     }else {
		 $param->{'files'}{$f}{'complete'} = $f;
     ...
     $param->{'file'} = $in{'file'};
     $param->{'complete'} = gettext($wwslib::filenames{$in{'file'}}{'gettext_id'});
	 ...
	 $filename_for_auth = 'info.file' if ($filename_for_auth eq 'info');
	 my ($role,$right) = $list->may_edit($filename_for_auth, $param->{'user'}{'email'});
 ...
 sub do_send_me {
	 ...
	 while (<MAIL>) {
		...
		$idline .= <MAIL>;
	     ...
	     if ($idline =~ /^Message-id\s*:\s*<?([^>\s]+)>?\s?/i ) {
		 my $id = $1;
		 if ($id eq $in{'msgid'}) {
		     $msgfile = $file ;
		 }
...
#################################################### 
 ...
 sub do_arcsearch {
	...
	my @directories = ();
	 ...
	 foreach my $dir (sort {$b cmp $a} grep(!/^\./,readdir ARC)) {
	     if ($dir =~ /^(\d{4})-(\d{2})$/) {
		if ($archive_name) {
		    push @directories, $dir if $dir le $archive_name;
		    $archive_name = '' if $dir lt $archive_name;
		}
		push @{$param->{'yyyymm'}}, $dir;
	     }
	 }
 ...
 sub do_get_closed_lists {
    ...
    my $all_lists = &List::get_lists($robot,
				     { 'filter_query' => [ 'status' => 'closed|family_closed' ] });
    foreach my $list ( @$all_lists ) {
...
## WWSympa Home-Page
 ...
 sub do_reviewbouncing {
     &wwslog('info', 'do_reviewbouncing(%s)', $in{'page'});
...
sub do_edit_list {
	...
	&report::reject_report_web('intern', 'sync_include_admin_failed',
				   {}, $param->{'action'}, $list,
...
####################################################
 sub do_request_topic {
     ...
     my $head = $msg->head();
     # headers will be encoded later.
     $param->{'subject'} = tools::decode_header($head, 'Subject');
     $param->{'subject'} = &tools::escape_html($param->{'subject'});
     $param->{'from'} = tools::decode_header($head, 'From');
     $param->{'from'} = &tools::escape_html($param->{'from'});
     $param->{'date'} = tools::decode_header($head, 'Date');
...
# manage blacklist
sub do_blacklist {
    &wwslog('info', 'do_blacklist(%s)', $param->{'list'});
...
sub get_protected_email_address {
 ...
 ## 
sub do_viewlogs {
	&wwslog('info', 'do_viewlogs(%s)', $in{'page'});
	
	$param->{'page'} = int($in{'page'}) || 1;
	$param->{'size'} = int($in{'size'}) || $wwsconf->{'viewlogs_page_size'};
	$param->{'sortby'} = $in{'sortby'} || 'email';
	...
	if($in{'target_type'} or $in{'page'} or $in{'size'}) {
		...
		$param->{'total_results'} = $#{$param->{'log_entries'}} + 1;
		$param->{'total_results'} = &Log::return_rows_nb() if(&Log::return_rows_nb() != 0);
		
		unless($param->{'total_results'}) {
			&report::reject_report_web('user','no_logs',{},$param->{'action'});
			&wwslog('info','do_viewlogs: no results');
			return 1;
		}
		
		$param->{'total_page'} = int($param->{'total_results'} / $param->{'size'});
		$param->{'total_page'}++ if($param->{'total_results'} % $param->{'size'});
		...
		@{$param->{'log_entries'}} = sort { lc $a->{$param->{'sortby'}} cmp lc $b->{$param->{'sortby'}} } @{$param->{'log_entries'}};
		
		my $last = $offset + $param->{'size'};
		$last = $param->{'total_results'} - 1 if($last >= $param->{'total_results'});
		@{$param->{'log_entries'}} = @{$param->{'log_entries'}}[$offset..$last];
	}
...
sub do_rss_request {
	...
	if ($sub_is =~ /reject/) {
	    &wwslog('info', 'RSS not accessible because list %s is not visible to user %s',$list->get_list_id,$param->{'user'}{'email'});
	    &web_db_log({'parameters' => $param->{'user'}{'email'},
			 'status' => 'error',
			 'error_type' => 'authorization'});		      
	    return undef;
...
sub do_automatic_lists_management_request {
    ...
    $param->{'automatic_lists_description'} = &Conf::load_automatic_lists_description();
    return 1;
}
...
sub do_automatic_lists_request {
    ...
    unless ($family->is_allowed_to_create_automatic_lists(('auth_level' => 'md5', 'sender' => $session->{'email'}, 'message' => undef, 'listname' => ''))){
	&report::reject_report_web('auth',"You are not allowed to create list in this family",{},$param->{'action'});
	&wwslog('err', 'Access to automatic list creation form denied to user %s.', $session->{'email'});
	return undef;
    }
...
sub do_automatic_lists {
    ...
    my @list_name_parts;
    my $families_config = &Conf::get_robot_conf($robot,'automatic_list_families');
    my $family_config = $families_config->{$family_name};
    my $listname = $family_config->{'prefix'}.$family_config->{'prefix_separator'};
	...
	unless ($list = $family->create_automatic_list(('listname' => $listname,'auth_level' => 'md5','sender'=>$session->{'email'}))) {
	    &Log::do_log('err', "Failed to create the dynamic list $listname.");
	    &List::send_notify_to_listmaster('automatic_list_creation_failed',$robot,["Failed to create the dynamic list $listname."]);
	    return undef;
	}
    }
...
sub prevent_visibility_bypass {
	...
	if ($sub_is =~ /reject/) {
	    &wwslog('info', 'visibility: List must remain hidden. Returning "home" to prevent visibility bypass');
	    return "home";
	}else {
	    return undef;
	}
